...
def sort_text(detection_boxes, detection_labels):
    ...
    # YOLO classes: 6: id, 8: name, 1: dob, 10: origin_place, 0: current_place
    id_boxes = detection_boxes[detection_labels == 6]
    name_boxes = detection_boxes[detection_labels == 8]
    birth_boxes = detection_boxes[detection_labels == 1]
    home_boxes = detection_boxes[detection_labels == 10]
    add_boxes = detection_boxes[detection_labels == 0]

    # Logical spatial filtering: Nơi thường trú (add_boxes) must be below Quê quán (home_boxes)
    # Find the average Y of home_boxes
    if len(home_boxes) > 0 and len(add_boxes) > 0:
        avg_home_y = np.mean([box[0] for box in home_boxes]) # ymin
        
        # Filter add_boxes: keep only those whose ymin is at least close to or below avg_home_y
        valid_add_boxes = []
        for box in add_boxes:
            if box[0] >= avg_home_y - 15: # allow small margin
                valid_add_boxes.append(box)
            else:
                # If an add_box is above home_boxes, it's likely a misclassified home_box
                # Convert it to home_box!
                home_boxes = np.vstack([home_boxes, box])
        
        add_boxes = np.array(valid_add_boxes) if valid_add_boxes else np.array([])
        
        # Re-evaluate avg_add_y after potential changes
        if len(add_boxes) > 0 and len(home_boxes) > 0:
            avg_add_y = np.mean([box[0] for box in add_boxes])
            valid_home_boxes = []
            for box in home_boxes:
                if box[0] <= avg_add_y + 15:
                    valid_home_boxes.append(box)
                else:
                    # Misclassified add_box
                    add_boxes = np.vstack([add_boxes, box])
            home_boxes = np.array(valid_home_boxes) if valid_home_boxes else np.array([])

    # arrange boxes
    id_boxes = sort_each_category(id_boxes) if len(id_boxes) > 0 else []
    name_boxes = sort_each_category(name_boxes) if len(name_boxes) > 0 else []
    birth_boxes = sort_each_category(birth_boxes) if len(birth_boxes) > 0 else []
    home_boxes = sort_each_category(home_boxes) if len(home_boxes) > 0 else []
    add_boxes = sort_each_category(add_boxes) if len(add_boxes) > 0 else []
